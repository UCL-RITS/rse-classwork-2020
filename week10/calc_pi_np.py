...
def point_in_circle(array_xy, radius=1):
    """
    ...
    """
    r = np.sqrt(np.sum(np.square(array_xy), axis = 1)) # axis = 1 means sums the columns and 0 means rows
    return r <= radius
...
def command():
    ...
    calc_pi = calculate_pi_timeit(1000)
